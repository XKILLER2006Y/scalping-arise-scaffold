...
def eval_pullback_cont(analysis: dict, feats: dict, closes: list[float] | None = None) -> dict:
    ...
    direction = "LONG" if trend == "UPTREND" else ("SHORT" if trend == "DOWNTREND" else None)
    if direction:
        met.append(f"underlying trend={trend}")
    else:
    ...
    near_sr = px is not None and atr and (
        (sup is not None and abs(px - sup) <= 0.5 * atr) or
        (res is not None and abs(px - res) <= 0.5 * atr))
...
def evaluate_all(analysis: dict, features: dict, close: float | None = None,
                 ...
                 source_type: str = "SPOT") -> list[dict]:
    ...
    feats["_close"] = close
    out = []
    for sid, ev in (("TREND_CONT", eval_trend_cont(analysis, feats)),
                    ("PULLBACK_CONT", eval_pullback_cont(analysis, feats, closes)),
                    ("RANGE_FADE", eval_range_fade(analysis, feats, entry_price=close))):
